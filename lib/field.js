...
function Field(property, label) {
  ...
  this.run = function (source, form, options) {
    ...
    if (Field.language != options.language) {
      if (typeof options.language === 'object') {
        Field.MESSAGES = options.language
      } else {
        Field.MESSAGES = require('./i18n/'+ options.language);
        if (! Field.MESSAGES) {
          throw new Error('Language "'+ options.language +'" is not defiend');
        }
      }
    }
...
Field.MESSAGES = require('./i18n/en') // default
Field.language = 'en'
...
Field.prototype.arrLength = function (from, to) {
  return this.add(function (arr) {
    if (value.length < from) {
      return { error: message || e.message || Field.MESSAGES.arrLength.short };
    }
    if (value.length > to) {
      return { error: message || e.message || Field.MESSAGES.arrLength.long };
    }
    return { valid: true };
  });
}
...
Field.prototype.custom = function(func, message) {
  return this.add(function (value, source) {
    try {
      ...
      return { error: message || e.message || Field.MESSAGES.custom };
    }
...
Object.keys(ValidatorPrototype).forEach(function (name) {
  ...
  Field.prototype[name] = function (message) {
    ...
    return this.add(function(value) {
      try {
        ValidatorPrototype[name].apply(externalValidator.check(value, message || Field.MESSAGES[name]), args);
      } catch (e) {
...
Field.prototype.contains = function (test, message) {
  return this.add(function(value) {
    try {
      ...
      return { error: message || Field.MESSAGES.contains };
    }
    return { valid: true };
  });
};
...
Field.prototype.notContains = function (test, message) {
  return this.add(function (value) {
    try {
      ...
      return { error: message || Field.MESSAGES.notContains };
    }
    return { valid: true };
  });
};
...
Field.prototype.equals = function (other, message) {
  ...
  return this.add(function (value, source) {
    ...
    if (value != test) {
      // TODO support multiple string arguments
      return { error: message || Field.MESSAGES.equals + String(test) };
    }
    return { valid: true };
  });
};
...
Field.prototype.isNumeric = function (message) {
  return this.add(function (value) {
    if (object.isNumber(value) || (object.isString(value) && value.match(/^[-+]?[0-9]*\.?[0-9]+$/))) {
      ...
      return { error: message || Field.MESSAGES.isNumeric };
    }
    return { valid: true };
  });
};
...
Field.prototype.isFloat = Field.prototype.isDecimal = function (message) {
  return this.add(function (value) {
    if ((object.isNumber(value) && value % 1 == 0) || (object.isString(value) && value.match(/^[-+]?[0-9]*\.[0-9]+$/))) {
      ...
      return { error: message || Field.MESSAGES.isDecimal };
    }
    return { valid: true };
  });
};
...
Field.prototype.regex = Field.prototype.is = function (pattern, modifiers, message) {
  ...
  return this.add(function (value) {
    if (pattern.test(value) === false) {
      return { error: message || Field.MESSAGES.regex };
    }
    return { valid: true };
  });
};
...
Field.prototype.notRegex = Field.prototype.not = function(pattern, modifiers, message) {
  ...
  return this.add(function(value) {
    if (pattern.test(value) === true) {
      return { error: message || Field.MESSAGES.notRegex };
    }
    return { valid: true };
  });
};
...
Field.prototype.required = function (placeholderValue, message) {
  ...
  return this.add(function (value) {
    if (!utils.hasValue(value) || value == placeholderValue) {
      return { error: message || Field.MESSAGES.required };
    }
    return { valid: true };
  });
};
...
Field.prototype.minLength = function (length, message) {
  return this.add(function(value) {
    if (value.toString().length < length) {
      return { error: message || Field.MESSAGES.minLength };
    }
    return { valid: true };
  });
};
...
Field.prototype.maxLength = function (length, message) {
  return this.add(function(value) {
    if (value.toString().length > length) {    
      return { error: message || Field.MESSAGES.maxLength };
    }
    return { valid: true };
  });
};
